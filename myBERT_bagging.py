...
def bagging_vote(pred_list): ## Based on the predictions of each bert, get the final predictions by voting.
    voted_pred_list = []
    num_pred = len(pred_list[0])
    for i in range(num_pred):
        vote_dict = {}
        for j in range(len(pred_list)):
            if pred_list[j][i] in vote_dict.keys():  ## pred_list[j][i] is the prediction of the j th model on the i th data.
                vote_dict[pred_list[j][i]] += 1
            else:
                vote_dict[pred_list[j][i]] = 1
        vote_list = [(key, value) for key, value in vote_dict.items()]
        sorted_vote_list = sorted(vote_list, key=lambda x: x[1], reverse=True)
        voted_pred_list.append(sorted_vote_list[0][0])
    return voted_pred_list

def mybootstrap(x):
    num = len(x)
    x = np.array(x)
    sample = resample(x,n_samples=num)
    sample = list(sample)
...
def load_data():
    ...
    return x_train, y_train, x_dev, dev_article_ids, dev_span_starts, dev_span_ends, label_to_idx_dict, idx_to_label_dict

  
def myBERT(x_train,y_train,x_dev):
...
def output_file(y_dev_pred,dev_article_ids, dev_span_starts, dev_span_ends):
...
def myBERT_bagging(num_bert=7):
    x_train, y_train, x_dev, dev_article_ids, dev_span_starts, dev_span_ends, label_to_idx_dict, idx_to_label_dict= load_data()
    pred_list = []
    print("----------------------There are in total {} models to vote.------------------".format(num_bert))
    for i in range(num_bert):
        xy_train = list(zip(x_train, y_train))
        xy_train = mybootstrap(xy_train)
        x_train, y_train = zip(*xy_train)
        x_train = list(x_train)
        y_train = list(y_train)
        for j in range(len(y_train)):
            y_train[j]=int(y_train[j])
        print("Model {} starts.".format(i+1))
        y_dev_pred = myBERT(x_train, y_train, x_dev)
        print("Model {} is doned.".format(i+1))
        pred_list.append(y_dev_pred)
    y_final_dev_pred = bagging_vote(pred_list)
    y_dev_pred = idx_to_label(y_dev_pred,idx_to_label_dict)        
    output_file(y_final_dev_pred, dev_article_ids, dev_span_starts,dev_span_ends)
